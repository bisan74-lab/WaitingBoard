...
- **관리자 대시보드** (`/admin`):
  - 대기/호출/오는중 목록, 통계, 경과 대기 시간, **연락처 표시**
  - **📞 전화** 버튼(`tel:` 링크) — 손님에게 바로 전화
  - **📨 호출** — 카카오톡 발송. **응답이 없으면 1분마다 자동 재호출**
  - **🚶 오는중** / 손님의 "지금 갈게요" 응답 시 **자동 재호출 중지**
  - **✅ 처리완료**(착석) — 목록에 남되 **취소선 + 그레이** 처리
  - **자동 갱신**(폴링, 서버리스 호환), **QR 코드** 표시·인쇄, **설정 + 연결 상태 점검**
- 손님 등록 화면 우측 상단 **⚙︎ 아이콘**으로 관리자 모드 전환, **모바일 대응** UI

### 대기 상태 흐름

`대기중 → 호출중(1분마다 재호출) → 오는중(재호출 중지) → 처리완료(취소선·그레이)`
손님은 자기 순번 화면(`/status/:id`)에서 **"지금 갈게요"** 를 눌러 직접 오는중으로 응답할 수 있습니다.

### 자동 재호출 동작 방식

- **로컬/미니PC(상시 서버)**: 서버 내장 타이머가 브라우저 없이도 1분마다 재호출합니다. (모니터 없는 헤드리스 장비에 적합)
- **Vercel(서버리스)**: 열려있는 관리자 대시보드가 `/api/cron/recall` 을 주기적으로 호출하거나, Vercel Cron(유료 플랜은 분 단위) + `CRON_SECRET` 으로 트리거합니다.
- 환경변수 `RECALL_INTERVAL_MS`(간격), `RECALL_MAX_ATTEMPTS`(최대 횟수, 0=무제한)로 조정.
...
## 매장 설치(미니PC / Windows 7 단말)

- **모니터 없는 미니PC**는 **서버 전용**으로 쓰기 좋습니다. Node만 설치해 `npm start` 로 상시 실행하면(모니터 불필요), 서버 내장 타이머가 자동 재호출까지 처리합니다.
  - 관리자 화면은 같은 네트워크의 **휴대폰·태블릿·다른 PC 브라우저**에서 `http://<미니PC_IP>:3000/admin` 으로 접속.
  - Windows에서 상시 실행/자동 시작은 [PM2](https://pm2.keymetrics.io/) 나 `nssm` 으로 서비스 등록을 권장.
- **Windows 7 단말**에서 관리자 화면을 볼 경우: 최신 크롬/엣지는 Win7 지원을 종료했으므로 **Firefox ESR 115**(권장) 또는 **Chrome 109** 를 설치하면 현재 화면이 그대로 동작합니다. (IE11 고정 환경이면 별도 ES5 버전이 필요합니다.)
